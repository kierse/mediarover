	...
	if config:
		...
		if _have_write_permission(os.path.join(path, "series_filter.conf")):
			...
			for series in series_list:
				data += subsection.format(series=series)
				...
				for filter, value in items:
					...
					data += filters.format(filter=filter, value=value)

			# add closing comment
			data += "\n# Generated by Media Rover v{version} on {time}\n".format(
		...
		if not os.path.exists(os.path.join(path, "series_filter.conf")):
			...
			print "Created {0}".format(os.path.join(path, "series_filter.conf"))
